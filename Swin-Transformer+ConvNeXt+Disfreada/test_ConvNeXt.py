"""
测试脚本（ConvNeXt 版本 - 跨中心评估）

在目标域（sw_2zibo）上评估已训练的 ConvNeXt 最佳模型，输出完整的分类报告、
混淆矩阵、ROC 曲线，以及错误预测样本列表。

用法：
    python test_ConvNeXt.py
    python test_ConvNeXt.py --ckpt outputs_convnext/checkpoints/best_model.pth
    python test_ConvNeXt.py --model_name convnext_small
"""
...
def parse_args():
    p = argparse.ArgumentParser(description="ConvNeXt 分类器 测试脚本")
    p.add_argument(
        "--ckpt",
        type=str,
        default=None,
        help="检查点路径（默认使用 outputs_convnext/checkpoints/best_model.pth）",
    )
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="ConvNeXt 模型名，需与训练时一致（默认 convnext_tiny）",
    )
    return p.parse_args()
...
@torch.no_grad()
def run_inference(model: ConvNeXtClassifier, loader: DataLoader, device: str):
    """
    对目标域数据集进行完整推理。
...
def plot_confusion_matrix(cm: np.ndarray, save_path: str):
    ...
    ax.set_ylabel("True")
    ax.set_title("Confusion Matrix - ConvNeXt (Target Domain)")
    fig.tight_layout()
...
def plot_roc_curve(labels: list, probs: list, save_path: str):
    ...
    ax.set_title("ROC Curve - ConvNeXt (Target Domain)")
...
def main():
    ...
    ckpt_path = args.ckpt or os.path.join(
        convnext_output_dir, "checkpoints", "best_model.pth"
    )
    if not os.path.isfile(ckpt_path):
        raise FileNotFoundError(
            f"未找到检查点文件：{ckpt_path}\n"
            "请先运行 train_ConvNeXt.py 完成训练，或通过 --ckpt 指定路径。"
        )
    ...
    model = ConvNeXtClassifier(
        ...
        pretrained  = False,   # 测试时不重新加载 ImageNet 权重
    ).to(device)
    ...
    print("\n" + "=" * 55)
    print("  ConvNeXt 跨中心测试结果（目标域：sw_2zibo）")
    print("=" * 55)
    ...
    # 保存结果                                                             #
    # ------------------------------------------------------------------ #
    result_dir = os.path.join(convnext_output_dir, "test_results")
