...
简单的监督学习：
    输入图像 → Swin Transformer → 分类头 → 交叉熵损失
...
def parse_args():
    p = argparse.ArgumentParser(description="Swin Classifier 训练脚本")
...
def train_one_epoch(
    ...
    for batch in train_loader:
        ...
        # 计算损失
        loss, loss_details = criterion(logits, cls_labels)
...
@torch.no_grad()
def validate(model: SwinClassifier, val_loader, device: str) -> dict:
    model.eval()
